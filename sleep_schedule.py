...
existing_row_idx = next((i + 2 for i, row in enumerate(df_records) if str(row.get("date")) == str(entry_date)), None)
...
if not st.session_state.df.empty:
    ...
    def average_time(times):
        ...
        return time(int(avg_seconds // 3600) % 24, int((avg_seconds % 3600) // 60))
    ...
    if not filtered_df.empty:
        ...
        fig = px.line(duration_chart, x="date", y="Sleep Duration (hrs)", markers=True, color_discrete_sequence=["#028283"])
        ...
        fig.update_layout(
            ...
            yaxis=dict(range=[duration_chart["Sleep Duration (hrs)"].min()-0.5, duration_chart["Sleep Duration (hrs)"].max()+0.5], showgrid=False),
