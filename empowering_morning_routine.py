...
ws = client.open("empowering_morning_routine").sheet1

if "morning_routine_df" not in st.session_state:
    st.session_state.morning_routine_df = pd.DataFrame(ws.get_all_records())
...
st.title("☀️ Empowering Morning Routine")

st.subheader(today.strftime('%B %d, %Y'))

if not st.session_state.morning_routine_df.empty:
    df = st.session_state.morning_routine_df.copy()
    
    for idx, row in df.iterrows():
        routine_key = f"routine_{idx}"
        routine_name = str(row.get('empowering_morning_routine', '')).strip()
        ...
        if add_clicked:
            if new_routine.strip():
                try:
                    ...
                    st.success(f"Added '{new_routine}' to your morning routines!")
                    st.session_state.morning_routine_df = pd.DataFrame(ws.get_all_records())
    ...
    total_items = len([row for _, row in df.iterrows() if str(row.get('empowering_morning_routine', '')).strip()])
    ...
    st.info("No morning routines yet. Click 'Manage Routines' below to add your first routine!")
